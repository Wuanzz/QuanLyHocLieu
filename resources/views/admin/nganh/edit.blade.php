        ...
        <div class="col-lg-8">
            <div class="card shadow-sm border-0 rounded-4">
                <div class="card-header bg-white border-0 py-3 px-4">
                    <h4 class="mb-0 fw-bold text-dark"><i class="fa-solid fa-pen-to-square text-primary me-2"></i>Sửa thông tin Ngành</h4>
                </div>
                <div class="card-body p-4 p-md-5">
                    
                    <form action="{{ route('admin.nganh.update', $nganh->NganhID) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group mb-4">
                            ...
                            <select id="KhoaID" name="KhoaID" class="form-select form-control-lg shadow-sm rounded-3 bg-light border-0 px-3" required>
                                ...
                                @foreach($danhSachKhoa as $khoa)
                                    <option value="{{ $khoa->KhoaID }}" {{ old('KhoaID', $nganh->KhoaID) == $khoa->KhoaID ? 'selected' : '' }}>
                                        {{ $khoa->TenKhoa }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="text" id="TenNganh" name="TenNganh" value="{{ old('TenNganh', $nganh->TenNganh) }}" class="form-control form-control-lg shadow-sm rounded-3 bg-light border-0 px-3" required />
                            @error('TenNganh')
                                <span class="text-danger small fw-bold mt-1 d-block">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-5">
                            ...
                            <textarea id="MoTa" name="MoTa" class="form-control shadow-sm rounded-3 bg-light border-0 px-3 py-2" rows="5" placeholder="Nhập mô tả cho ngành này...">{{ old('MoTa', $nganh->MoTa) }}</textarea>
                        </div>
